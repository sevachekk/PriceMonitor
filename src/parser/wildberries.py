...
async def get_wildberries_product_price(product_name: str) -> int:
    """
    Возвращает цену ПЕРВОГО товара на Wildberries.ru.
    Если «Ничего не найдено» — возвращает 0.
    """
    encoded_name = quote(product_name)
    search_url = f"https://www.wildberries.ru/catalog/0/search.aspx?search={encoded_name}"
    ...
    async with async_playwright() as p:
        ...
        try:
            cookie_file = find_cookie_file("cookie_wb.txt")
            if cookie_file is None:
                raise FileNotFoundError(
                    f"cookie_wb.txt; searched: {', '.join(get_cookie_search_locations('cookie_wb.txt'))}"
                )
            ...
            if not isinstance(cookies_data, list):
                raise ValueError("Неверный формат cookie_wb.txt")

            cleaned_cookies = clean_cookies(cookies_data)
            await context.add_cookies(cleaned_cookies)
            print("✅ Cookies успешно загружены и очищены из cookie_wb.txt")

        except FileNotFoundError as exc:
            print(f"⚠️ {exc}. Продолжаем без cookies.")
        except Exception as e:
            print(f"⚠️ Ошибка при загрузке cookie_wb.txt: {e}")
        ...
        try:
            await page.wait_for_selector("ins.price__lower-price", timeout=15000)
        except PlaywrightTimeoutError:
            await browser.close()
            raise TimeoutError("Не найдена цена товара и не обнаружено сообщение «Ничего не найдено»")

        price_locator = page.locator("ins.price__lower-price").first
