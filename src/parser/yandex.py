...
async def get_yandex_product_price(product_name: str):
    """
    Возвращает цену ПЕРВОГО товара в поисковой выдаче Яндекс Маркета.
    Если товаров нет («Такого у нас нет») — возвращает 0.
    """
    encoded_name = quote(product_name)
    search_url = f"https://market.yandex.ru/search?text={encoded_name}"
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await browser.close()
            raise TimeoutError("Не найдена ни цена товара, ни сообщение «Такого у нас нет»")
